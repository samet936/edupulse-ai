...
class EduPulseMasterEngine:
    ...
    def build_strategy_logic(self):
        ...
        if score >= 85: 
            ...
            problem = "Düşük uyku und yüksek kaygı bilişsel kapasiteyi kilitliyor."
...
def index():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if veriler and isinstance(veriler, dict):
                for key, deger in veriler.items():
                    if isinstance(deger, dict):
                        gecmis_analizler.append(deger)
                gecmis_analizler.reverse()
    except Exception as e:
        print("Firebase veri çekme hatası:", e)
        
    return render_template('index.html', analizler=gecmis_analizler)
...
def analyze():
    ...
    try:
        requests.post(FIREBASE_URL, json=firebase_veri, timeout=5)
    except Exception as e:
